...
def property_assignment(custom_body, instance, keypath, property, property_type) 
	...
	elsif property_type == "Font"
		right_side = "#{property}.font"
    elsif property_type == "Color?"
        right_side = "#{property}?.color"
    elsif property_type == "Font?"
        right_side = "#{property}?.font"
	else
...
def generate_swift_style(os, availability, class_name, keypath, property, property_type, custom_body, style_name, comment)
...
STYLE
    if availability then 
        code = "@available(#{os} #{availability}, *)\n" + code 
    end
    code = "/** #{comment} */\n" + code
end
...
def generate_extension(class_name, styles, platform)
    extension = <<-EXTENSION
public extension #{class_name} {
#{styles.map { |s| generate_swift_style(platform, s["available"], class_name, s["keypath"], s["property"], s["type"], s["custom_body"], s["style_name"], s["comment"]).margin(4) }.join("\n")}}
EXTENSION
    extension
end 
...
styles.each do |class_name, class_styles|
    ...
    extension = generate_extension(class_name, class_styles, platform)
